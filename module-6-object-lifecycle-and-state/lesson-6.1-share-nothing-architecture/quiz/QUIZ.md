...
- A) Yes — the same instance spans all 50,000 rows, so `$errors` grows without bound and row 50,000 is processed by an object in a very different state from row 1.
- B) No — CLI scripts are exempt from lifecycle concerns because they do not serve HTTP requests.
- C) No — the script exits when it finishes, so share-nothing is preserved.
