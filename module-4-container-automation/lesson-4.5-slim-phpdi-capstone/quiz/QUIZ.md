...
| 12 | **F** | The three booleans are `displayErrorDetails`, `logErrors` and `logErrorDetails` — none of them selects a response format. Slim's error middleware **content-negotiates**: it inspects the request's `Accept` header and renders JSON for `application/json`, HTML for `text/html`, and so on. So a JSON client already gets JSON without this call, and setting all three to `false` only means "no stack traces in the response, nothing written to the log". If you want a specific error *shape* — an envelope matching your success responses, say — register a custom error handler with `setErrorHandler()`. |
