...
| 14 | **F** | Promotion is syntactic sugar: the property is declared, typed and assigned exactly as it would be by hand, and no behaviour differs. One caveat on the word *identical* — the engine does remember which form you used, and `(new ReflectionProperty(Svc::class, 'db'))->isPromoted()` returns `true` for one and `false` for the other. You will meet that API in Module 4. It changes nothing at runtime; it is simply the only way to tell them apart. |
